...
export default function MyPortfolios() {
    ...
    return (
        <main className="flex flex-col items-start justify-center min-h-screen text-white px-10 ml-20 -mt-24">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-y-11 gap-x-80">
                {/* {someValue && someValue.value && someValue.value[0].map((bigNumber) => (
                    <Card key={parseInt(bigNumber._hex, 16)} id={parseInt(bigNumber._hex, 16)} contract={contract}/>
                ))}  */}
                {
                    [0, 1].map((id) => 
                    <Card 
